...
use bevy_ggrs::{GgrsPlugin, AdvanceFrame, PlayerInputs, Rollback, Session, LocalInputResource};
...
fn it_runs_advance_frame_schedule_systems() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let inputs1 = HashMap::from([
        (player1.handle, BoxInput { inp: 0 }),
    ]);
    let inputs_resource = LocalInputResource::<GgrsConfig> {
        inputs: inputs1,
    };
...
fn create_app<T: Config + Default>(session: P2PSession<T>) -> App {
    ...
    app.add_plugins(MinimalPlugins)
        ...
        .add_plugin(GgrsPlugin::<T>::default())
        .insert_resource(Session::P2PSession(session))
        .insert_resource(FrameCount { frame: 0 })
        .add_systems(
            (move_player_system, increase_frame_system)
                .in_schedule(AdvanceFrame))
...
pub fn register_input_system(_handle: In<PlayerHandle>, keyboard_input: Res<Input<KeyCode>>) -> BoxInput {
    let mut input: u8 = 0;
